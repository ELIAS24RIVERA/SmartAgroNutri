...
export default function MonitorSuelo() {
  ...
  const [messages, setMessages] = useState([{ sender: "bot", text: "Hola 👋 Soy tu asistente verde. ¿En qué puedo ayudarte?" }]);
  ...
  const sendMessage = () => {
    ...
    setTimeout(() => {
      setMessages((prev) => [...prev, { sender: "bot", text: "Esto es una respuesta automática del asistente 🌱" }]);
    }, 500);
    setInputText("");
  };
  ...
  const calibrarConductividad = (raw) => {
    ...
    return Number(((raw / 4095) * 5000).toFixed(0));
  };

  const calibrarTemperatura = (raw) => {
    if (raw == null || isNaN(raw)) return "--";
    return Number((raw * 0.1 + 20).toFixed(1));
  };
  ...
  return (
    <>
      ...
      {/* === CHATBOT flotante === */}
      <div className="chatbot-toggle" onClick={() => setIsChatOpen(!isChatOpen)}>💬</div>
      {isChatOpen && (
        <div className="chatbot-container open">
          <div className="chatbot-header">🤖 Asistente Verde</div>
          <div className="chatbot-messages">
            {messages.map((msg, i) => (
              <div key={i} className={`chat-msg ${msg.sender === "user" ? "msg-user" : "msg-bot"}`}>
                {msg.text}
              </div>
            ))}
            <div ref={chatEndRef} />
